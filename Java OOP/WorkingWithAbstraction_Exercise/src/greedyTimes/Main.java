...
public class Main {
    public static void main(String[] args) {
        ...
        Map<> torba = new LinkedHashMap<String, LinkedHashMap<String, Long>>();
        long gold = 0;
        long stones = 0;
        long money = 0;
        ...
        for (int i = 0; i < seif.length; i += 2) {
            ...
            if (kvoE.equals("")) {
                continue;
            } else if (input < torba.values().stream().map(Map::values).flatMap(Collection::stream).mapToLong(e -> e).sum() + broika) {
                continue;
            }
